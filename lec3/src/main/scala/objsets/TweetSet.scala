...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def filterAcc(p: (Tweet) => Boolean, acc: TweetSet): TweetSet = {
    left.filterAcc(p, right.filterAcc(p, if (p(this.elem)) {
      acc.incl(elem)
    } else acc))
  }
  ...
  override def mostRetweeted: Tweet = {
    ...
    (left.isEmpty, right.isEmpty) match {
      case (true, true) => elem
      case (false, false) => max(left.mostRetweeted, right.mostRetweeted)
      case (true, false) => max(elem, right.mostRetweeted)
      case (false, true) => max(left.mostRetweeted, elem)
    }
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter((t) => {
    google.exists { key => t.text.contains(key) }
  })

  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter((t) => {
    apple.exists { key => t.text.contains(key) }
  })
