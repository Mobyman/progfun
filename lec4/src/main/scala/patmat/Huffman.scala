...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    trees match {
      case left :: right :: tail => (makeCodeTree(left, right) :: tail).sortWith((a, b) => {
        weight(a) < weight(b)
      })
      case single => single
    }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def loop(elems: CodeTree, bits: List[Bit]): List[Char] = {
      elems match {
        case l: Leaf =>
          if (bits.isEmpty)
            List(l.char)
          else
            l.char :: loop(tree, bits)

        case f: Fork =>
          if (bits.head == 0)
            loop(f.left, bits.tail)
          else
            loop(f.right, bits.tail)
      }
    }
  ...
  def loop(tree: CodeTree)(char: Char): List[Bit] = {
    tree match {
  ...
  def convert(tree: CodeTree): CodeTable = {
      ...
      tree match {
        ...
        case Leaf(char, _) => List( (char, loop(tree)(char)) )
      }
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def addBit(bit: Bit)(code: Code): Code = ( code._1 , bit :: code._2)
